#include "../../lib/sdcc/fcs80.h"

#define SPRITE_NUM 64

// グローバル変数
typedef struct {
    // スプライトの表示情報
    struct Obj {
        union {
            uint8_t raw[2];
            uint16_t val;
        } x;
        union {
            uint8_t raw[2];
            uint16_t val;
        } y;
        int8_t vx;
        int8_t vy;
        uint8_t an;
    } obj[SPRITE_NUM];
} GlobalVariables;
#define GV ((GlobalVariables*)0xC000)

// 乱数テーブル
const uint8_t random[256] = {
    
};

void main(void)
{
    // パレットを初期化
    fcs80_palette_set_rgb555(0, 0, 0b0000000000000000);
    fcs80_palette_set_rgb555(0, 1, 0b0001110011100111);
    fcs80_palette_set_rgb555(0, 2, 0b0110001100011000);
    fcs80_palette_set_rgb555(0, 3, 0b0111111111111111);
    fcs80_palette_set_rgb555(0, 4, 0b0000001110000000);
    fcs80_palette_set_rgb555(0, 5, 0b0000000000011100);

    // Bank 2 を Character Pattern Table (0xA000) に転送 (DMA)
    fcs80_dma(2);

    // FGの左上に "SPRITE TEST" を描画
    fcs80_fg_putstr(2, 2, 0x80, "SPRITE TEST");

    // BG全画面にひし形を描画
    for (int y = 0; y < 32; y++) {
        for (int x = 0; x < 32; x++) {
            FCS80_ADDR_BG->ptn[y][x] = 0x10;
            FCS80_ADDR_BG->attr[y][x] = 0x80 | ((x & 1) << 5) | ((y & 1) << 6);
        }
    }

    // スプライト & グローバル変数を初期化
    uint8_t rptr = 0;
    for (int i = 0; i < SPRITE_NUM; i++) {
        FCS80_ADDR_OAM[i].x = random[rptr++];
        FCS80_ADDR_OAM[i].y = random[rptr++] % 192;
        FCS80_ADDR_OAM[i].ptn = 1 + (i & 0x07);
        FCS80_ADDR_OAM[i].attr = 0x80;
        GV->obj[i].x.val = FCS80_ADDR_OAM[i].x << 8;
        GV->obj[i].y.val = FCS80_ADDR_OAM[i].y << 8;
        GV->obj[i].vx = random[rptr++];
        GV->obj[i].vy = random[rptr++];
        GV->obj[i].an = random[rptr++];
    }

    // メインループ
    while (1) {
        // V-BLANK を待機
        fcs80_wait_vsync();

        // スプライトを動かす
        for (int i = 0; i < SPRITE_NUM; i++) {
            GV->obj[i].x.val += GV->obj[i].vx;
            GV->obj[i].y.val += GV->obj[i].vy;
            FCS80_ADDR_OAM[i].x = GV->obj[i].x.raw[1];
            FCS80_ADDR_OAM[i].y = GV->obj[i].y.raw[1];
            GV->obj[i].an++;
            GV->obj[i].an &= 0x03;
            if (0 == GV->obj[i].an) {
                FCS80_ADDR_OAM[i].ptn++;
                if (9 == FCS80_ADDR_OAM[i].ptn) {
                    FCS80_ADDR_OAM[i].ptn = 1;
                }
            }
        }

        // BGをスクロール
        (*FCS80_ADDR_BG_SCROLL_Y)--;
    }
}